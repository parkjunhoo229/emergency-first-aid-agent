...
async def update_user(
    ...
    try:
        ...
        result = await db.execute(
            select(User)
            .options(selectinload(User.medical_info))
            .where(and_(User.id == user_id, User.is_active == True))
        )
        user = result.scalar_one_or_none()
        ...
        # 3) 의료 정보 업데이트 (새로 추가된 부분)
        if any([
            user_data.blood_type is not None,
            user_data.base_diseases is not None,
            user_data.medications is not None,
            user_data.allergies is not None,
            user_data.surgery_history is not None,
            user_data.other_medical_info is not None,
            user_data.emergency_contact_name is not None,
            user_data.emergency_contact_phone is not None,
            user_data.emergency_contact_relation is not None,
        ]):
            medical_info = user.medical_info

            # 의료 정보 레코드가 없으면 새로 생성
            if medical_info is None:
                blood_type = sanitize_string(
                    user_data.blood_type or "미등록",
                    10
                )
                medical_info = MedicalInfo(
                    user_id=user.id,
            ...
            # 각 필드별로 들어온 값만 업데이트
            if user_data.blood_type is not None:
                medical_info.blood_type = sanitize_string(user_data.blood_type, 10)

            if user_data.base_diseases is not None:
                medical_info.base_diseases = sanitize_string(user_data.base_diseases, 1000)

            if user_data.medications is not None:
                medical_info.medications = sanitize_string(user_data.medications, 1000)

            if user_data.allergies is not None:
                medical_info.allergies = sanitize_string(user_data.allergies, 1000)

            if user_data.surgery_history is not None:
                medical_info.surgery_history = sanitize_string(user_data.surgery_history, 1000)

            if user_data.other_medical_info is not None:
                medical_info.other_medical_info = sanitize_string(user_data.other_medical_info, 1000)

            if user_data.emergency_contact_name is not None:
                medical_info.emergency_contact_name = sanitize_string(user_data.emergency_contact_name, 50)

            if user_data.emergency_contact_phone is not None:
                # 빈 문자열일 수 있으니, 빈 값 들어오면 그냥 빈 문자열로 저장
                phone_raw = user_data.emergency_contact_phone.strip()
                medical_info.emergency_contact_phone = (
                    format_phone_number(phone_raw) if phone_raw else ""
                )

            if user_data.emergency_contact_relation is not None:
                medical_info.emergency_contact_relation = sanitize_string(user_data.emergency_contact_relation, 20)

            medical_info.updated_at = datetime.utcnow()

        # 4) 사용자 업데이트 시간 갱신
        user.updated_at = datetime.utcnow()

        await db.commit()
        # medical_info를 포함한 최신 상태 로드
        ...
        await db.rollback()
        # 여기 detail에 실제 에러 메시지까지 붙여서 디버깅용으로 사용
        raise HTTPException(
            status_code=500,
...
async def send_message(message_data: MessageSend, db: AsyncSession = Depends(get_db)):
    try:
        ...
        medical = user.medical_info

        # 1) 사용자 프로필 (한 번만 세팅, 없으면 만들어줌)
        if "user_profile" not in agent_state:
            agent_state["user_profile"] = {
                ...
                "medical_info": {
                    "blood_type": medical.blood_type if medical else None,
                    "base_diseases": medical.base_diseases if (medical and medical.base_diseases) else "",
                    "medications": medical.medications if (medical and medical.medications) else "",
                    "allergies": medical.allergies if (medical and medical.allergies) else "",
                    "other": medical.other_medical_info if (medical and medical.other_medical_info) else "",
                },
                "emergency_contact": {
                    "name": medical.emergency_contact_name if (medical and medical.emergency_contact_name) else "",
                    "phone": medical.emergency_contact_phone if (medical and medical.emergency_contact_phone) else "",
                    "relation": medical.emergency_contact_relation if (medical and medical.emergency_contact_relation) else "",
                },
            }

        # 2) GPS 정보는 매번 최신값으로 갱신
        if getattr(message_data, "lat", None) is not None and getattr(message_data, "lng", None) is not None:
            agent_state["gps_location"] = {
