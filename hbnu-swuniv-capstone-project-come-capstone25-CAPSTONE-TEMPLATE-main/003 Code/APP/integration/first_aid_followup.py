...
def run_first_aid_core(
    ...
    """
    응급처치 분기 판단 + 질문/지침 반환 로직의 본체.
    - 에이전트 내부에서는 이 함수를 직접 호출
    - HTTP 요청이 들어오면 followup_handler 라우터가 이 함수를 감싸서 사용
    반환 dict 형식:
        {
          "status": "진행중" | "확정" | "error",
          "question": str | None,
    ...
    if answer_history:
        history_text = "\n".join(
            ...
            for m in answer_history
        )
    else:
        history_text = "없음"

    sym_text = ", ".join(symptoms) if symptoms else "없음"
...
- 다음 데이터를 기준으로 각 분기의 조건과 일치 여부를 평가하라:
  1. 응급도 ({emergency_level})
  2. 증상 ({sym_text if symptoms else "증상 정보 없음"})
  3. 사용자 응답 히스토리 ({answer_history if answer_history else "없음"})
