...
def run_location_core(location_history: list[dict], user_input: str) -> dict:
    """
    위치 파악 GPT 호출 로직의 본체.
    - 에이전트 내부에서는 이 함수를 직접 호출
    - HTTP 요청이 들어오면 라우터에서 이 함수를 감싸서 사용
    반환 dict 형식:
        {
          "status": "진행중" | "확정" | "error",
          "followup_question": str | None,
          "final_location_text": str | None
        }
    """
...
- 건물명이나 동 이름이 음성 인식 오류로 약간 다르게 들려도, 사람이 보기에 어떤 건물인지 충분히 추정 가능하면 그대로 final_location_text로 사용해야 합니다. (예: "한밭대 에스동 1층" → "한밭대학교 S동 1층" 수준으로 이해하고 문장을 완성)
    ...
    try:
        ...
        if parsed is None:
            return {
                ...
                "final_location_text": "GPT 응답 파싱 실패",
            }
        ...
        if "final_location_text" in parsed:
            ...
            if user_turns >= 5:
                ...
                return {
                    "status": "확정",
                    "followup_question": None,
                    "final_location_text": approx_loc,
                }
        ...
        return {
            "status": "error",
            "followup_question": None,
            "final_location_text": f"GPT 호출 실패: {e}",
        }
...
def run_location(req: LocationRequest = Body(...)):
    data = run_location_core(req.location_history, req.user_input)
    return LocationResponse(
        status=data["status"],
        followup_question=data.get("followup_question"),
        final_location_text=data.get("final_location_text"),
    )