...
class _HospitalListScreenState extends State<HospitalListScreen> {
  ...
  Future<void> addHospitalMarkersToMap() async {
    ...
    for (int i = 0; i < hospitals.length; i++) {
      ...
      if (hospital.lat != null &&
        ...
        print(
          '[addHospitalMarkersToMap] 유효하지 않은 좌표: ${hospital.name} - lat: ${hospital.lat}, lng: ${hospital.lng}',
        );
      }
    }
  }
  ...
  void updateLocationOnMap() {
    if (position != null && mapController != null) {
      if (position!.latitude.isNaN || position!.longitude.isNaN) {
        print(
          '[updateLocationOnMap] 사용자 위치가 NaN입니다: lat=${position!.latitude}, lng=${position!.longitude}',
        );
        return;
      }
              ...
              child: NaverMap(
                ...
                onMapReady: (controller) {
                  ...
                  locationOverlay.setSubIcon(
                    NOverlayImage.fromAssetImage("assets/sub.png"),
                  );
  ...
  NCameraPosition _getInitialCameraPosition() {
    if (position != null &&
      ...
      if (position != null) {
        print(
          '[getInitialCameraPosition] 사용자 위치가 유효하지 않음: lat=${position!.latitude}, lng=${position!.longitude}',
        );
      }
