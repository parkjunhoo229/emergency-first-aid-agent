...
class CallScreen extends StatelessWidget {
  const CallScreen({Key? key}) : super(key: key); // 파라미터 싹 제거
  ...
  Widget build(BuildContext context) {
    ...
    // user가 null일 일은 거의 없지만, 혹시 몰라 방어 코드
    if (user == null) {
      return const Scaffold(body: Center(child: CircularProgressIndicator()));
    }

    // 화면에 표시할 값들을 Provider에서 읽어옴
    final name = user.name;
    final gender = user.gender;
    final birthYear = user.birthYear.toString();
    final bloodType = user.medicalInfo?.bloodType ?? '미등록';
    final baseDiseases = user.medicalInfo?.baseDiseases ?? '';
    final medications = user.medicalInfo?.medications ?? '';
    final allergies = user.medicalInfo?.allergies ?? '';
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () {
              // 이미 위에서 authProvider, user를 가져왔으니 그대로 이용
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder:
                      (context) => MyPageScreen(
                        ...
                        otherMedicalInfo:
                            user.medicalInfo?.otherMedicalInfo ?? '',
                        emergencyContactRelation:
                            user.medicalInfo?.emergencyContactRelation ?? '',
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (baseDiseases.isNotEmpty)
                      _buildInfoRow('기저질환', baseDiseases),
                    if (medications.isNotEmpty)
                      _buildInfoRow('복용약', medications),
                              ...
                              MaterialPageRoute(
                                builder:
                                    (context) => MyPageScreen(
                                      ...
                                      birthYear: user.birthYear.toString(),
                                      bloodType:
                                          user.medicalInfo?.bloodType ?? '미등록',
                                      baseDiseases:
                                          user.medicalInfo?.baseDiseases ?? '',
                                      medications:
                                          user.medicalInfo?.medications ?? '',
                                      allergies:
                                          user.medicalInfo?.allergies ?? '',
                                      surgeryHistory:
                                          user.medicalInfo?.surgeryHistory ??
                                          '',
                                      otherMedicalInfo:
                                          user.medicalInfo?.otherMedicalInfo ??
                                          '',
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        currentIndex: 0,
        items: const [
          BottomNavigationBarItem(icon: Icon(Icons.call), label: '응급 통화 걸기'),
