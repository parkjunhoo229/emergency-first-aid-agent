...
class VoiceChatService {
  ...
  static const String baseUrl = 'http://192.168.137.1:5000/api'; // Android
  ...
  static Future<Map<String, dynamic>?> sendMessage(String message) async {
    ...
    try {
      ...
      // 2) 위치 가져오기 시도 (실패해도 그냥 넘어가도록)
      try {
        final position = await GeolocatorHelper.getCurrentLocation();
        payload['lat'] = position.latitude;
        payload['lng'] = position.longitude;
        print('보낼 GPS → lat: ${position.latitude}, lng: ${position.longitude}');
      } catch (e) {
        // GPS 실패해도 채팅은 계속 가야 하니까 로그만 찍고 무시
        print('GPS 가져오기 실패, 위치 없이 전송합니다: $e');
      }
